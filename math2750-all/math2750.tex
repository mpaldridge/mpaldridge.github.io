...
Actually we're fine. The reason is that the first return to \(i\) isn't just any old random time, it's a ``stopping time'', and the Markov property also applies to stopping times too. Roughly speaking, a \textbf{stopping time} is a random time where ``you know when you get there''.

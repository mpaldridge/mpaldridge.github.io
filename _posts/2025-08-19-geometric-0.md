...
-   The PMF of the negative binomial distribution is $\displaystyle \left(\kern-0.4em\binom{n}{x}\right\kern-0.4em) \theta^{-x} (1 + \theta)^{-n-x}$, where $\left(\binom{n}{x}\right) = n^{\overline{x}} / x!$ is the multiset coefficient.

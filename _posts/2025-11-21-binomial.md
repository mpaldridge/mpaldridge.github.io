---
layout: post
title:  "Don’t write the binomial coefficient as n! / k! (n-k)!s"
date:   2025-11-21
categories: maths
...
$$ \newcommand{\multiset}[2]{\bigg(\kern-0.4em\dbinom{#1}{#2}\kern-0.4em\bigg)} $$
...
This formula, let’s call it **Formula B**, is also correct, in that is mathematically equal to Formula A. It's also much, much more popular than Formula A. But I think it's bad!
...
$$ \frac{n!}{(n-k)!} = \frac{n \times (n-1) \times \cdots \times (n-k+1) \times (n-k) \times (n-k-1) \times \cdots \times 2 \times 1}{\phantom{n \times (n-1) \times \cdots \times (n-k+1)} \times (n-k) \times (n-k-1) \times \cdots \times 2 \times 1} $$

ensures the right-hand tail of the top factorial cancels. But I dislike this -- it's the sort of clever-clever writing of of an expression in terms of other things that, yes, avoids defining notation for the falling factorial, but goes out of it's way to disguise what's actually going on. Reject such too-smart-by-half rewritings of easily interpretable expressions!
...
still holds for $|a| < |b|$, where here the binomial coefficient means the same thing as in Formula A:

$$ \binom{x}{k} = \frac{x^{\underline{k}}}{k!} = \frac{x(x-1)\cdots(x-k+1)}{k!} . $$

So, for example, with $b = 1$, $|a| < 1$, and $x = 1/2$, we have the correct formula
...
which doesn't work at all, since Formula B doesn't make sense when $n$ isn't an integer. (Admittedly, you can usually get around this one with the Gamma function.)
...
$$ \binom{n}{k} = \frac{n^{\underline{k}}}{k!} \qquad \multi{n}{k}  = \frac{n^{\overline{k}}}{k!} . $$

But writing them Formula B-style gives the much less pleasing to the eye

$$ \binom{n}{k} = \frac{n!}{k! \, (n-k)!} \qquad \multi{n}{k} = \frac{(n+k-1)!}{k!\,(n-1)!} . $$
...
I sort of see the point. But really, I think that Formula B makes this *too obvious*. The symmetry relation is a deep an important fact, that deserves a proper explanation; just saying "It's clear: Formula B says so!" denies any possibility of understanding *why* the symmetry relation holds.

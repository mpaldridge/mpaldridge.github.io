...
In our examples of the simple random walk and gambler's ruin, we specified the start point \(X_0 = x_0\) exactly, but we could pick the start point at random according to some distribution \(\lambda_i = \mathbb P(X_0 = i)\).
...
\textbf{2.} Consider the two-state ``broken printer'' Markov chain, with state space \(\mathcal S = \{0,1\}\), transition matrix
